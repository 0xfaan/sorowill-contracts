...
fn distribute_large_balance_no_overflow() {
    ...
    let beneficiaries: SorobanVec<Beneficiary> = vec![
        &env,
        Beneficiary { address: beneficiary_a.clone(), allocation: Allocation::Percentage(3_333) },
        Beneficiary { address: beneficiary_b.clone(), allocation: Allocation::Percentage(6_667) },
    ];
    ...
    assert_eq!(total_distributed, large_amount, "total distributed should equal the will balance");
...
fn distribute_small_percentage_large_balance() {
    ...
    let beneficiaries: SorobanVec<Beneficiary> = vec![
        &env,
        Beneficiary { address: beneficiary_small.clone(), allocation: Allocation::Percentage(1) },
        Beneficiary { address: beneficiary_large.clone(), allocation: Allocation::Percentage(9_999) },
    ];
    ...
    assert!(balance_small > 0, "even tiny percentage should result in nonzero amount");
